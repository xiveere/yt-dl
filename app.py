...
from youtube_downloader import download_youtube_flac
import os
import time
from typing import List

st.set_page_config(
    page_title="YouTube FLAC Downloader",
    page_icon="🎵",
    layout="centered"
)

st.title("YouTube FLAC Downloader 🎵")
st.markdown("Download YouTube videos in high-quality FLAC format")
...
if st.session_state.download_queue and st.button("Download Queue", type="primary"):
    ...
    for i, url in enumerate(st.session_state.download_queue):
        status_text.text(f"Downloading {i+1}/{len(st.session_state.download_queue)}: {url}")
        try:
            flac_path = download_youtube_flac(url, output_dir)
...
if st.session_state.completed_downloads:
    ...
    for download in st.session_state.completed_downloads:
        if download['status'] == 'success':
            st.success(f"✅ {os.path.basename(download['path'])}")
            st.info(f"Saved to: {download['path']}")
        else:
            st.error(f"❌ Failed to download: {download['url']}")
